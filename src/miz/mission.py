...
class BaseMissionObject(Logged):
    def __init__(self, mission_dict: OrderedDict, l10n: OrderedDict):
        super().__init__()

        if not isinstance(mission_dict, OrderedDict):
            raise TypeError('mission_dict should be an OrderedDict, got: {}'.format(type(mission_dict)))

        if not isinstance(l10n, OrderedDict):
            print(l10n)
            raise TypeError('l10n should be an OrderedDict, got: {}'.format(type(l10n)))
            ...
            if group_category in self._section_this_country.keys():
                for group_index in self._section_this_country[group_category]['group']:
                    if group_index not in self.__groups[group_category]:
                        self.__groups[group_category][group_index] = Group(self.d, self.ln10, self.coa_color,
    ...
    def units(self):
        for unit_index in self._section_group['units']:
            if unit_index not in self.__units.keys():
                self.__units[unit_index] = self.units_class_enum[self.group_category](self.d, self.ln10, self.coa_color,
                                                                                      self.country_index,
                                                                                      self.group_category,
