...
class Weather():
    ...
    def retrieve_online_metar(self, location):
        ...
        html = request_get('https://www.aviationweather.gov/adds/tafs?station_ids={}&std_trans=standard&submit_taf=Get+TAFs'.format(location.lower())).content.decode('utf8')
        ...
        return
        # print(html)
        re_taf_line = re_compile(r'.*LATEST TAF REPORT FOR {}\s+(?P<taf>.*)\s+LATEST TAF REPORT FOR {}.*'.format(location, location), re_MULTILINE)
