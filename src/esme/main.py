...
from os import stat, remove, removedirs, _exit, startfile
from os.path import exists, abspath, join, normpath, basename, dirname
from copy import deepcopy, copy
# noinspection PyUnresolvedReferences
from operator import itemgetter
from re import compile as re_compile
# noinspection PyUnresolvedReferences
from PyQt5.QtCore import QThread, pyqtSlot as Slot, pyqtSignal as Signal, QObject, Qt, QRegExp, QAbstractTableModel, QModelIndex, QVariant, QSortFilterProxyModel, QAbstractListModel, \
    QAbstractItemModel
# noinspection PyUnresolvedReferences
from PyQt5.QtGui import QImage, QIcon, QPixmap, QBrush, QRegExpValidator, QStandardItemModel, QStandardItem
# noinspection PyUnresolvedReferences
from PyQt5.QtWidgets import QMainWindow, QApplication, QMessageBox, QPushButton, QWidget, QStyledItemDelegate, QLineEdit, QStyleOption, QComboBox, QStyleOptionViewItem, QDoubleSpinBox
...
# Own imports
from ui import main as qt_main_ui, widget_units as qt_widget_units, widget_config as qt_widget_config, widget_radios as qt_widget_radios, widget_roster as qt_widget_roster, \
    widget_static as qt_widget_static, widget_flights as qt_widget_flights
...
VALID_CLIENT_AC = {'A-10A', 'A-10C', 'F-15C', 'FW-190D9', 'ka-50', 'L-39ZA', 'mi-8mt', 'mig-29a', 'mig-29s', 'P-51D', 'su-25', 'su-25t', 'su-27', 'su-33', 'TF-51D', }
...
class BuiltinsWrapper(Logged):
    ...
    def excepthook(_type, value, tback):
        ...
        """
        LOGGER.error('excepthook intercept error:\n{}: {}\n{}'.format(_type, value, ''.join([x for x in traceback.format_tb(tback)])))
...
class Utils(BaseUtils):
    ...
    def send_crash_report(crash_program=True):
        ...
        Utils.send_mail(subject='{} crash report'.format(__app_name__), body='', secure=SECURE,
                        text_attachment_as_text=[(debug_text, 'debug'), (jdumps(Config().state.data, indent=True, sort_keys=True), 'config')], _from=_from)
        if crash_program:
            exit(1)
...
class UnitsWidget(AbstractWidget, qt_widget_units.Ui_Form):
        ...
        def filterAcceptsRow(self, row_num, index):
            ...
            if not self.coa == 'any':
                if not self.coa == basemodel.data(basemodel.index(row_num, basemodel.headers.index('Coalition'))):
                    return False
            if self.client and not basemodel.data(basemodel.index(row_num, basemodel.headers.index('Skill'))) == 'Client':
                return False
            return True
    ...
    class DelegateUnitOrGroupName(QStyledItemDelegate):
        ...
        def validate_new_name(self, model, col, new_name, existing=None):
            ...
            if new_unit_name_lower in existing:
                ...
                if m:
                    return self.validate_new_name(model, col, '{} #{}'.format(m.group('base_name'), str(int(m.group('counter')) + 1).zfill(3)), existing)
                else:
    ...
    def update_model(self):
        if not MIZ.is_loaded:
            return
        if MIZ.mission:
            unit_list = [[unit.group_id, unit.group_name, unit.unit_id, unit.unit_name, unit.group_category, unit.unit_type, unit.skill, unit.coa_color] for unit in MIZ.mission.units]
            ...
            self.table_units.setItemDelegateForColumn(self.model.headers.index('Group name'), UnitsWidget.DelegateUnitOrGroupName(self.table_units))
            self.table_units.setItemDelegateForColumn(self.model.headers.index('Unit name'), UnitsWidget.DelegateUnitOrGroupName(self.table_units))
            self.table_units.setItemDelegateForColumn(self.model.headers.index('Skill'), UnitsWidget.DelegateUnitSkill(self.table_units))
...
class RadiosWidget(AbstractWidget, qt_widget_radios.Ui_Form):
    ...
    def __init__(self, parent, linked_btn):
        super().__init__(parent)
        self.btns = [self.btn_huey, self.btn_ka, self.btn_mi, self.btn_sabre, self.btn_mirage, self.btn_mig21, self.btn_p51, self.btn_tf51, self.btn_all]
    ...
    def aircraft_list_clicked(self):
        ...
        radio1_model = RadiosWidget.RadioTableModel([[channel, freq] for channel, freq in radio1.channels], headers, self.table_radio1)
        radio1_model.sort()
        self.label_radio1.setText(radio1.radio_name)
        self.table_radio1.setModel(radio1_model)
        self.radio1_delegate = RadiosWidget.FrequencyEditor(radio1.min, radio1.max, self.table_radio1, self.double_r1_step)
        ...
        try:
            ...
            radio2_model = RadiosWidget.RadioTableModel([[channel, freq] for channel, freq in radio2.channels], headers, self.table_radio2)
            radio2_model.sort()
            ...
            self.radio2_delegate = RadiosWidget.FrequencyEditor(radio2.min, radio2.max, self.table_radio2, self.double_r2_step)
            self.table_radio2.setItemDelegateForColumn(1, self.radio2_delegate)

    def set_radio2_enabled(self, enabled=True):
        for e in [self.label_radio2, self.table_radio2, self.btn_r2_apply, self.btn_r2_apply_to_all, self.btn_r2_copy, self.btn_r2_discard, self.btn_r2_load, self.btn_r2_paste, self.btn_r2_save,
...
class RosterWidget(AbstractWidget, qt_widget_roster.Ui_Form):
    ...
    def set_roster_file_path(self):
        new_path = Utils.user_select('Choose the roster file (must be a valid roster file if it already exists)', 'save_file', str_filter='*.roster')
    ...
    def __add_pilot(self, pilot_name, parent_name):
        ...
        try:
            pilot = Pilot(pilot_name, parent, default_ac=AC(parent.default_ac.name), default_skin=Skin(parent.default_skin.name))
...
class Gui(AbstractGui, qt_main_ui.Ui_MainWindow):
    ...
    def gather_skins(self):
        ...
        self.widget_config.label_skin_scan_result.setText('Found {} skins'.format(sum(len(x) for _, x in skins.available_skins.items())))
