...
DO_LARGE_TESTS = os.environ.get('EMFT_LARGE_TEST', None) == '1'


class TestMizFile:
    def __init__(self, miz_file_path):
        self.p = Path(miz_file_path)
        self.src = self.p.abspath()
        self.out = Path(self.p.dirname()).joinpath('{}_EMFT.miz'.format(self.p.namebase)).abspath()
...
class TestMizBasics:
    def test_init(self, tmpdir):
        Miz(TEST_FILE, temp_dir=str(tmpdir))
        with pytest.raises(FileNotFoundError):
            Miz('./i_do_not_exist', temp_dir=str(tmpdir))

    def test_context(self, tmpdir):
        with Miz(TEST_FILE, temp_dir=str(tmpdir)) as miz:
            assert isinstance(miz.mission, Mission)
            assert isinstance(miz.l10n, OrderedDict)
            assert miz.temp_dir_path == str(tmpdir)
            assert miz.ln10_file_path == Path(str(tmpdir)).joinpath('l10n', 'DEFAULT', 'dictionary').abspath()
            assert miz.mission_file_path == Path(str(tmpdir)).joinpath('mission').abspath()

    def test_unzip(self, tmpdir):
        miz = Miz(TEST_FILE, temp_dir=str(tmpdir))
        miz.unzip()

    def test_decode(self, tmpdir):
        miz = Miz(TEST_FILE, temp_dir=str(tmpdir))
        miz.unzip()
        miz.decode()

    @pytest.mark.skipif(not DO_LARGE_TESTS, reason='skipping large tests')
    def test_large_decode(self, tmpdir):
        test_miz = TestMizFile('./test/test_files/TRMT_2.4.0.miz')
        miz = Miz(test_miz.src, temp_dir=str(tmpdir))
        miz.unzip()
        miz.decode()
    ...
    def test_zip(self, tmpdir):
        with Miz(TEST_FILE, temp_dir=str(tmpdir)) as miz:
            ...
            with Miz(OUT_FILE, temp_dir=str(tmpdir)) as miz2:
                assert miz.mission.d == miz2.mission.d
                miz.mission.weather.cloud_density = 4
                assert not miz.mission.d == miz2.mission.d

    @pytest.mark.skipif(not DO_LARGE_TESTS, reason='skipping large tests')
    def test_large_zip(self, tmpdir_factory):
        trmt = TestMizFile('./test/test_files/TRMT_2.4.0.miz')
        with Miz(trmt.src, temp_dir=str(tmpdir_factory.mktemp())) as miz:
            miz.zip()
            out_file = trmt.out
            assert Path(out_file).exists()
            with Miz(out_file, temp_dir=str(tmpdir_factory.mktemp())) as miz2:
                assert miz.mission.d == miz2.mission.d
                miz.mission.weather.cloud_density = 4
                assert not miz.mission.d == miz2.mission.d
                sleep(1)
                m1 = miz.mission_file_path
                m2 = miz2.mission_file_path
    ...
    def test_is_unzipped(self, tmpdir):
        mis = Miz(TEST_FILE, temp_dir=str(tmpdir))
        assert not mis.is_unzipped
        mis.unzip()
        assert mis.is_unzipped

    def test_missing_file_in_miz(self, tmpdir):
        missing = Miz(MISSING_FILE, temp_dir=str(tmpdir))
        with pytest.raises(FileNotFoundError):
            missing.unzip()

    def test_bad_zip_file(self, tmpdir):
        mis = Miz(BAD_ZIP_FILE, temp_dir=str(tmpdir))
        with pytest.raises(BadZipFile):
            mis.unzip()

    def test_temp_dir_cleaning(self, tmpdir):
        mis = Miz(TEST_FILE, temp_dir=str(tmpdir))
        mis.unzip()
        p = Path(mis.temp_dir_path)
        assert p.exists()
        assert not p.listdir() == []
        mis.remove_temp_dir()
        assert not p.exists()
...
class TestMizValues:
    ...
    def test_ln10(self, miz):
        assert miz.l10n == OrderedDict(
            [('DictKey_UnitName_72', 'Unit #019'), ('DictKey_UnitName_60', 'Unit #015'),
             ('DictKey_WptName_58', ''), ('DictKey_WptName_52', ''), ('DictKey_GroupName_11', 'gilles'),
             ('DictKey_sortie_4', 'sortie_test'), ('DictKey_GroupName_23', 'New Vehicle Group #003'),
             ('DictKey_GroupName_17', 'New Vehicle Group #001'), ('DictKey_GroupName_8', 'gal'),
             ('DictKey_WptName_76', ''), ('DictKey_GroupName_77', 'New Vehicle Group #021'),
             ('DictKey_GroupName_68', 'New Vehicle Group #018'), ('DictKey_WptName_19', ''),
             ('DictKey_WptName_22', ''), ('DictKey_UnitName_39', 'Unit #008'), ('DictKey_WptName_100', ''),
             ('DictKey_WptName_70', ''), ('DictKey_UnitName_12', 'gilles'),
             ('DictKey_GroupName_71', 'New Vehicle Group #019'), ('DictKey_UnitName_45', 'Unit #010'),
             ('DictKey_WptName_46', ''), ('DictKey_UnitName_102', 'Unit #029'), ('DictKey_WptName_43', ''),
             ('DictKey_UnitName_9', 'gal'), ('DictKey_WptName_13', ''),
             ('DictKey_GroupName_62', 'New Vehicle Group #016'), ('DictKey_UnitName_108', 'Unit #031'),
             ('DictKey_UnitName_87', 'Unit #024'), ('DictKey_GroupName_44', 'New Vehicle Group #010'),
             ('DictKey_WptName_64', ''), ('DictKey_WptName_106', ''), ('DictKey_UnitName_75', 'Unit #020'),
             ('DictKey_UnitName_54', 'Unit #013'), ('DictKey_UnitName_36', 'Unit #007'),
             ('DictKey_WptName_109', ''), ('DictKey_descriptionText_1', ''),
             ('DictKey_GroupName_107', 'New Vehicle Group #031'),
             ('DictKey_GroupName_32', 'New Vehicle Group #006'),
             ('DictKey_GroupName_101', 'New Vehicle Group #029'), ('DictKey_WptName_16', ''),
             ('DictKey_UnitName_69', 'Unit #018'), ('DictKey_UnitName_66', 'Unit #017'),
             ('DictKey_WptName_82', ''), ('DictKey_WptName_25', ''), ('DictKey_WptName_79', ''),
             ('DictKey_UnitName_6', 'etcher'), ('DictKey_WptName_112', ''), ('DictKey_WptName_91', ''),
             ('DictKey_WptName_31', ''), ('DictKey_UnitName_51', 'Unit #012'),
             ('DictKey_UnitName_33', 'Unit #006'), ('DictKey_GroupName_14', 'New Vehicle Group'),
             ('DictKey_WptName_40', ''), ('DictKey_WptName_85', ''), ('DictKey_WptName_94', ''),
             ('DictKey_GroupName_74', 'New Vehicle Group #020'), ('DictKey_WptName_28', ''),
             ('DictKey_WptName_73', ''), ('DictKey_descriptionRedTask_2', ''),
             ('DictKey_UnitName_21', 'Unit #002'), ('DictKey_UnitName_90', 'Unit #025'),
             ('DictKey_UnitName_48', 'Unit #011'), ('DictKey_descriptionBlueTask_3', ''),
             ('DictKey_UnitName_99', 'Unit #028'), ('DictKey_WptName_55', ''), ('DictKey_WptName_97', ''),
             ('DictKey_UnitName_111', 'Unit #032'), ('DictKey_GroupName_50', 'New Vehicle Group #012'),
             ('DictKey_GroupName_47', 'New Vehicle Group #011'), ('DictKey_WptName_10', ''),
             ('DictKey_GroupName_98', 'New Vehicle Group #028'),
             ('DictKey_GroupName_20', 'New Vehicle Group #002'), ('DictKey_WptName_37', ''),
             ('DictKey_GroupName_80', 'New Vehicle Group #022'), ('DictKey_UnitName_57', 'Unit #014'),
             ('DictKey_GroupName_29', 'New Vehicle Group #005'), ('DictKey_UnitName_105', 'Unit #030'),
             ('DictKey_UnitName_27', 'Unit #004'), ('DictKey_GroupName_35', 'New Vehicle Group #007'),
             ('DictKey_WptName_49', ''), ('DictKey_UnitName_24', 'Unit #003'),
             ('DictKey_GroupName_38', 'New Vehicle Group #008'), ('DictKey_UnitName_18', 'Unit #001'),
             ('DictKey_GroupName_83', 'New Vehicle Group #023'),
             ('DictKey_GroupName_95', 'New Vehicle Group #027'),
             ('DictKey_GroupName_110', 'New Vehicle Group #032'), ('DictKey_UnitName_42', 'Unit #009'),
             ('DictKey_WptName_103', ''), ('DictKey_GroupName_65', 'New Vehicle Group #017'),
             ('DictKey_WptName_67', ''), ('DictKey_UnitName_78', 'Unit #021'),
             ('DictKey_UnitName_15', 'Unit #1'), ('DictKey_UnitName_93', 'Unit #026'),
             ('DictKey_GroupName_53', 'New Vehicle Group #013'),
             ('DictKey_GroupName_26', 'New Vehicle Group #004'),
             ('DictKey_GroupName_86', 'New Vehicle Group #024'), ('DictKey_UnitName_81', 'Unit #022'),
             ('DictKey_GroupName_92', 'New Vehicle Group #026'), ('DictKey_WptName_88', ''),
             ('DictKey_GroupName_5', 'etcher'), ('DictKey_UnitName_63', 'Unit #016'),
             ('DictKey_GroupName_41', 'New Vehicle Group #009'), ('DictKey_WptName_7', ''),
             ('DictKey_GroupName_59', 'New Vehicle Group #015'), ('DictKey_UnitName_96', 'Unit #027'),
             ('DictKey_GroupName_56', 'New Vehicle Group #014'), ('DictKey_WptName_61', ''),
             ('DictKey_UnitName_30', 'Unit #005'), ('DictKey_UnitName_84', 'Unit #023'),
             ('DictKey_GroupName_89', 'New Vehicle Group #025'), ('DictKey_WptName_34', ''),
    ...
    def test_turbulence(self, miz):
        ...
        assert miz.mission.weather.turbulence_at8000 == 0
        assert miz.mission.weather.turbulence_at2000 == 0
        for i in range(0, 60, 1):
            miz.mission.weather.turbulence_at2000 = i
            miz.mission.weather.turbulence_at8000 = i
        ...
        for wrong_turbulence in [-1, 61, True, None, 'caribou']:
            with pytest.raises(ValueError, msg=wrong_turbulence):
                miz.mission.weather.turbulence_at2000 = wrong_turbulence
                miz.mission.weather.turbulence_at8000 = wrong_turbulence
        ...
        for invalid_category in ('caribou', 'Plane', 'plAne', 'ships', -1, 0, 1, True, False, None):
            with pytest.raises(ValueError):
                for _ in miz.mission.blue_coa.get_groups_from_category(invalid_category):
                    pass
        with Miz('./test/test_files/all_objects.miz', temp_dir=miz.temp_dir_path) as miz:
    ...
    def test_set_name(self, tmpdir):
        with Miz(TEST_FILE, temp_dir=str(tmpdir)) as miz:
            ...
            assert isinstance(group, Group)
            group.group_name = 'new_etcher'
            miz._encode_ln10()
            miz.zip(OUT_FILE)
        with Miz(OUT_FILE, temp_dir=str(tmpdir)) as miz:
            new_group = miz.mission.get_group_by_name('new_etcher')
            assert isinstance(new_group, Group)
            for attrib in ('group_category', 'group_index', 'group_hidden', 'group_start_time', '_group_name_key'):
                assert getattr(group, attrib) == getattr(new_group, attrib)

    def test_set_hidden(self, tmpdir):
        with Miz(TEST_FILE, temp_dir=str(tmpdir)) as miz:
            group = miz.mission.get_group_by_name('etcher')
            assert isinstance(group, Group)
    ...
    def test_group_start_time(self, tmpdir):
        with Miz(TEST_FILE, temp_dir=str(tmpdir)) as miz:
            group = miz.mission.get_group_by_name('etcher')
        ...
        with Miz(miz.default_destination, temp_dir=miz.temp_dir_path) as miz:
            group = miz.mission.get_group_by_name('etcher')
            assert group.group_start_delay == 3600 * 24
    ...
    def test_objects(self, tmpdir):
        with Miz(ALL_OBJECTS, str(tmpdir)) as miz:
            ...
            for wrong_category in (True, -1, 0, 1, False, None, 'caribou'):
                ...
                with pytest.raises(ValueError, msg=wrong_category):
                    for _ in country.get_units_from_category(wrong_category):
                        pass
            for id in (1, 2, 3, 4):
                assert isinstance(country.get_group_by_id(id), Group)
                assert isinstance(country.get_unit_by_id(id), BaseUnit)
    ...
    def test_get_country(self, miz):
        ...
        with Miz(TEST_FILE, temp_dir=miz.temp_dir_path) as miz:
            assert isinstance(miz.mission.blue_coa.get_country_by_name('USA'), Country)
    ...
    def test_sortie_name(self):
        with Miz(TEST_FILE) as miz:
            ...
            miz.zip(OUT_FILE)
        with Miz(OUT_FILE, temp_dir=miz.temp_dir_path) as miz:
